...
class Library {
  ...
  removeBook(bookID) {
    this.bookArray.splice(
      this.bookArray.findIndex((book) => book.id === bookID),
      1
    );
  }
}
...
const ScreenController = (function () {
  ...
  displayBooks = function () {
    ...
    myLibrary.bookArray.forEach((thisBook) => {
      //console.log(thisBook);
      const bookCard = createBookCard(thisBook);
  ...
  changeBookReadStatus = function () {
    ...
    let bookIndex = myLibrary.bookArray.findIndex((book) => book.id === bookID);
    myLibrary.bookArray[bookIndex].isRead =
      !myLibrary.bookArray[bookIndex].isRead;
  };
